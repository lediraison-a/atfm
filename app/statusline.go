...
func (m *StatusLine) Draw(screen tcell.Screen) {
	for _, element := range m.displayConfig.StatusLineElements {
		...
		elementStyle := style.NewStyle().Padding(1).Highlight(element.Highlight)

		switch strings.ToUpper(element.Name) {
		case "INDEX":
			t = m.displayIndex(elementStyle)
		case "FILENAME":
			t = m.displayCurrentFileName(elementStyle)
		case "FILEINFO":
			t = m.displayCurrentFileInfo(elementStyle)
		default:
			t = element.Name
		}
...
func (m *StatusLine) displayCurrentFileName(style *style.Style) string {
	...
	if ins.CanShowOpenParent() && ins.CurrentItem == 0 {
		t = ""
	}
	return style.Render(t)
}
